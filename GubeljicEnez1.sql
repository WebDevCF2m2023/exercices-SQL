...
SELECT c.*, GROUP_CONCAT(news.idnews) AS idnews, GROUP_CONCAT(news.title SEPARATOR '|||') AS title  FROM categ c
LEFT JOIN news_has_categ nhc ON c.idcateg = nhc.categ_idcateg
LEFT JOIN news n ON nhc.news_idnews = n.idnews
WHERE c.idcateg = 5
AND n.visible = 1
...
FROM news n
LEFT JOIN user ON n.user_iduser = u.iduser
WHERE title LIKE 'a%' AND n.visible = 1;
